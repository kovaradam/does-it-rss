import { DocumentQuery, getDocumentQuery } from "./utils";
import { DefinitionResult } from "./parse-feeds";

export function parseFeedsFromOpml(query: DocumentQuery): DefinitionResult[] {
  const { body } = query.extract({
    body: {
      selector: "body",
      value: {
        outlines: [
          {
            ...
            value(el) {
              ...
              return {
                title: elQuery.attr("text"),
                description: elQuery.attr("description"),
                url: elQuery.attr("xmlUrl"),
              };
            },
          },
        ],
...
export function getIsOpmlFile(query: DocumentQuery) {
  return query("opml").length === 1;
}

if (import.meta.vitest) {
  const { test, expect } = import.meta.vitest;

  test("should return true for opml file", () => {
    const query = getDocumentQuery(OPML_FILE);
    expect(getIsOpmlFile(query)).toBe(true);
  });

  test("should return definition results", () => {
    const query = getDocumentQuery(OPML_FILE);
    const result = parseFeedsFromOpml(query);

    expect(result.length).toBe(13);

    expect(result[0]?.content.title).toBe("CNET News.com");

    expect(result[0]?.content.description).toBe(
    ...
    expect(result[0]?.url.href).toBe("http://news.com.com/2547-1_3-0-5.xml");
  });
